...
SocialProvider.prototype = {
  ...
  init: function(windowCreatorFn) {
    ...
    Services.obs.notifyObservers(null, "social-service-init-ready", this.name);
  },
  shutdown: function() {
    try {
      this._log("shutdown");
    ...
    this.active = false;
    Services.obs.notifyObservers(null, "social-service-shutdown", this.name);
  },
  activate: function() {
    if (this.enabled) {
      this.init();
      this.active = true;
      Services.obs.notifyObservers(null, "social-service-activated", this.name);
    }
  },
  deactivate: function() {
    closeWindowsForService(this);
    this.active = false;
    Services.obs.notifyObservers(null, "social-service-deactivated", this.name);
  ...
  makeWorker: function(window) {
    ...
    if (!this.enabled) {
      throw new Error("cannot use disabled service "+this.name);
    }
    if (this.workerURL) {
      ...
      throw new Error("makeWorker cannot create worker: no workerURL specified for "+this.name);
    }
  },
