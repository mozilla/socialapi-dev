...
ProviderRegistry.prototype = {
  ...
  register: function(manifest) {
    ...
    dump("registering "+manifest.name+"\n");
    try {
      this._providers[manifest.origin] = new SocialProvider(manifest);
      if (!this._currentProvider)
        this._currentProvider = this._providers[manifest.origin];
    } catch(e) {
      Cu.reportError(e);
    }
    ...
    try {
      ...
      if (this._providers[currentProviderOrigin]) {
        this._currentProvider = this._providers[currentProviderOrigin];
      }
    } catch(e) {
      // just during dev, otherwise we shouldn't log here
      //Cu.reportError(e);
  ...
  set currentProvider(provider) {
    ...
    Services.obs.notifyObservers(null, "social-service-changed", provider.name);
  },
  get: function pr_get(origin) {
    return this._providers[origin];
  },
  getNamed: function pr_getNamed(name) {
