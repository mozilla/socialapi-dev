...
function _startup(options) {
  ...
  // Read a preference for the user's preferred service...
  let newWindowSidebarService = null;
...
function setVisibleService(window, aService)
{
  ...
  try {
    ...
    sbrowser.watcher = new SidebarLocationWatcher(sbrowser.service.URLPrefix, sbrowser);
...
function shouldShowSidebarForTab(window, tab) {
  // for now we just check if the url bar is visible.
  // XXX - todo - check URLs, such as about:* etc?
  // This is how browser.js checks visibility:
  // http://mxr.mozilla.org/mozilla-central/source/browser/base/content/browser.js#2185
  var bo = window.gURLBar.boxObject;
  return bo.height > 0 && bo.width > 0;
}

// do whatever needs to be done so we detect when the "visibility" state
// of the sidebar needs to change.
function watchForVisibilityChanges(window) {
  // So there are 2 cases we care about here WRT tab loading/switching.
  // * A tab is selected.
  // * The already selected tab changes its "load state" (eg, the tab is
  // selected as it is loading, or the tab changes location.)
  // So: setup a "tab selected" notification, then for the selected tab we
  // add a progress listener.
  let {document, gBrowser} = window;
  let checkSidebarVisibility = function(browser) {
    let newHidden = !shouldShowSidebarForTab(window, browser);
    mainLog("sidebar's desired visiblity is " + !newHidden);
    let vbox = document.getElementById("social-vbox");
    let splitter = document.getElementById("social-splitter");
    vbox.setAttribute("hidden", newHidden);
    splitter.setAttribute("hidden", newHidden);
  }
  let currentProgressListenerTab = null;
  let currentProgressListener = null;
  let tabSelectedListener = function(event) {
    try {
      // remove the progress listener from the old tab.
      if (currentProgressListener) {
        currentProgressListenerTab.webProgress.removeProgressListener(currentProgressListener);
      }
      let browser = gBrowser.selectedBrowser;
      // check the tab now...
      checkSidebarVisibility(browser);
      // and arrange to check when the tab document state changes.

      currentProgressListenerTab = browser;
      currentProgressListener = new TabStateWatcher(function(aWebProgress, aRequest, aStateFlags, aStatus) {
        // it isn't completely clear when the progress state reflects the actual state of the url bar.
        // Sometimes we need to catch "stop", but waiting that long might cause "flashing"
        // So we also check start so we can remove it ASAP when possible.
        if ((aStateFlags & (Ci.nsIWebProgressListener.STATE_STOP | Ci.nsIWebProgressListener.STATE_STOP)) &&
            (aStateFlags & Ci.nsIWebProgressListener.STATE_IS_DOCUMENT)) {
          mainLog("a tab's loadState changed (" + browser.currentURI.spec + ")");
          checkSidebarVisibility(browser);
        }
      });
      browser.webProgress.addProgressListener(currentProgressListener,
                                              Ci.nsIWebProgress.NOTIFY_STATE_DOCUMENT);
    } catch (ex) {
        mainLog("tabSelected listener failed: " + ex + "\n" + ex.stack);
    }
  }
  gBrowser.tabContainer.addEventListener("TabSelect", tabSelectedListener, false);
  // and assuming we missed the "select" event for the current tab, handle that.
  if (gBrowser.selectedBrowser) {
    tabSelectedListener();
  }
  // remove listeners as we unload
  unload(function() {
    gBrowser.tabContainer.removeEventListener("TabSelect", tabSelectedListener, false);
    if (currentProgressListener) {
      currentProgressListenerTab.webProgress.removeProgressListener(currentProgressListener);
    }
  }, window);
...
function attachSidebar(window)
{
  ...
  watchForVisibilityChanges(window);
...
function GenericLocationWatcher() {
  return this;
}


GenericLocationWatcher.prototype = {
...
// A location watcher for the sidebar.
function SidebarLocationWatcher(prefix, browser) {
  GenericLocationWatcher.call(this);
  this._prefix = prefix;
  this._browser = browser;
  return this;
}

SidebarLocationWatcher.prototype = {
  __proto__: GenericLocationWatcher.prototype,
...
// A location watcher for all tabs in a single browser.
function TabStateWatcher(stateChangeCB) {
  this._stateChangeCB = stateChangeCB;
  GenericLocationWatcher.call(this);
  return this;
}

TabStateWatcher.prototype = {
  __proto__: GenericLocationWatcher.prototype,
  ...
  onStateChange: function(/*in nsIWebProgress*/ aWebProgress,
                     ...
                     /*in nsresult*/ aStatus)
  {
    this._stateChangeCB(aWebProgress, aRequest, aStateFlags, aStatus);
  },
