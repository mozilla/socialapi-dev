...
function _startup(options) {
  ...
  watchWindows(function(window) {
    try {
      ...
      var perWindowServiceReady = function(svcname) {
        mainLog("service " + svcname + " is ready - initializing it");
        var service = getServiceNamed(svcname);
        if (service) {
      ...
      if (newWindowSidebarService) {
        let newWindowService= getServiceNamed(newWindowSidebarService);
        if (newWindowService && newWindowService.active) {
          perWindowServiceReady(newWindowService.name);
        }
      }
      // If it's not ready to go, take it when it's ready
      Services.obs.addObserver(function(subject, topic, data) {
        mainLog("Observed social-service-init-ready: " + data);
        if (data == newWindowSidebarService) {
          perWindowServiceReady(data);
        }      
  ...
  registry.get(function(manifest) {
    ...
    // If the user has no preferred service, just use the first one
    if (!newWindowSidebarService) newWindowSidebarService = service.name;

    service.init(null,
      function() {
        mainLog("service init completed: " + service.name);
    });
  });
...
function buildSocialPopupContents(window, socialpanel)
{
  ...
  try {
    ...
    // Create network rows...
    providerRegistry().iterate(function(service) {
