...
Cu.import("resource://socialdev/lib/jewels.js");
...
// We run a heartbeat timer which pings each service every
// HEARTBEAT_TIMER msec.
let gHeartbeatTimer = null;
...
function createHeartbeatTimer()
{
  gHeartbeatTimer = Cc["@mozilla.org/timer;1"].createInstance(Ci.nsITimer);
  gHeartbeatTimer.initWithCallback(function() {

    // we wish we had a global array of worker references
    // because making these workers isn't cheap
    for each (var service in gSocialServices) {
      let worker = service.makeWorker();
      worker.port.postMessage({topic: "heartbeat"});
      worker.port.close();
    }

  }, HEARTBEAT_TIMER, gHeartbeatTimer.TYPE_REPEATING_SLACK);

  unload(function() {
    gHeartbeatTimer.cancel();
  });
}
...
function createWindow(toURL, name, options, withService, title, readyCallback)
{
  ...
  let opts = {
    ...
    onClose: function() {
      ...
      withService.windowClosing(aWind); //.browser.contentWindow.wrappedJSObject);
    },
